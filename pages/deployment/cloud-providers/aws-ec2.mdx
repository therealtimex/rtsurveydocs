...
Customise by changing the `export` lines before launching.

### Option B — Full script via AWS CLI

The 16 KB limit does not apply when uploading via the CLI. Download [aws-ec2.sh](/scripts/aws-ec2.sh), edit the `CONFIGURATION` block at the top, then launch:
...
5. Click **Launch instance** (Option A) or run the CLI command (Option B)
