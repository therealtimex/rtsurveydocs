...
![AUDIO question type appearance variants](/images/xlsform/question-types/audio-part1.png)
...
| Appearance | Effect |
|---|---|
| `results{hide(capture)}` | Hides the record button after audio is captured |
| `results{hide(review)}` | Hides the playback button |
| `results{hide(filename)}` | Hides the filename and duration |
| `results{left}` | Moves icon to the left after capture |

Example — hide record button after capture, keep playback:
```
