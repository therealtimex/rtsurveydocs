...
The color controls the **button background**. Text color is always `var(--accent-color)` — it is not configurable via the appearance. If the `<#RRGGBB/>` token is omitted entirely, the button background falls back to `var(--primary-color)`.
