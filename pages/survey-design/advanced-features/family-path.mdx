...
- **Local database search** — the `db_path` argument of `search-autocomplete-noedit-v2('rawquery', ...)` must be built with `concat(${family_path}, ...)`. See [Local Database Search](local-database-search) for full usage details.
