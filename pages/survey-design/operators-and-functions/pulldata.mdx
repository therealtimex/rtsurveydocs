...
<Callout type="info">
`rawquery` is commonly used with autocomplete fields for hierarchical location lookups. See [Local Database Search](../advanced-features/local-database-search) for details on building `.db` files and wiring them to `pulldata('rawquery', ...)`.
</Callout>
