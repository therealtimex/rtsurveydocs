...
2. `string-length(field)`: Returns the length of a string field.
   ...
   - **Cascade dependency pattern**: `string-length(${province}) >= 0` always evaluates to `true` but forces rtSurvey to re-evaluate the expression whenever `${province}` changes. Use this in `relevant` to trigger a cascade refresh when a parent field updates:
     | type | name | relevant |
     |------|------|----------|
     | select_one | district | `string-length(${province}) >= 0` |
     This ensures the district question re-evaluates its `search()` or `rawquery` appearance whenever province changes.
...
8. `STRFTIME(format, datetime)`: SQLite-style date formatting. Available inside SQL strings passed to `rawquery` queries — **not** available in XLSForm `calculation` or `label` columns. Uses SQLite format tokens (`%Y`, `%m`, `%d`, `%H`, `%M`, `%S`).
   - Example inside a rawquery SQL string: `'SELECT STRFTIME(''%Y-%m'', interview_date) AS month FROM data WHERE id = ?'`
   - Note: Use `format-date-time()` for date formatting in XLSForm expressions. `STRFTIME()` is a SQLite function available only within SQL passed to `rawquery`.
...
5. `substr-jsonpath(value, jsonpath)`: Extracts a value from a JSON string using a JSONPath expression.
   - `value` must be a JSON string (e.g. a field that stored a raw API response using `callapi(..., 'json:', ...)`).
   - `jsonpath` is a JSONPath expression (e.g. `'$.data.name'`, `'$.items[0].id'`).
   - Always returns a **string** — wrap in `number()` when used in arithmetic.
   - Returns an empty string `''` if the path is not found or `value` is not valid JSON.
   - Example: `substr-jsonpath(${api_response}, '$.data.name')` — extract the `name` field.
   - Example: `substr-jsonpath(${api_response}, '$.items[0].label')` — first element of an array.
   - Example: `number(substr-jsonpath(${api_response}, '$.price'))` — extract a numeric value.
   - Typically used alongside `callapi(..., 'json:', ...)` to extract multiple fields from one API response. See [Fetching multiple values from one API call](../advanced-features/call-api#fetching-multiple-values-from-one-api-call).
