...
| `tagging-choices-noshow-v2-{c1}-{c2}-{c3}-{c4}-{c5}` | select_one, select_multiple | Renders choices as coloured tag chips. The five colour slots follow the same order as `rating_box-fill`. Selected choices are highlighted; unselected choices are suppressed from the summary view. |
