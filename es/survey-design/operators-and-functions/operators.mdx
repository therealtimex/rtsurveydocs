...
Ejemplo 1:
<Info>`${age} = 3 or ${age} = 4` devolverá `true` si la edad es 3 o 4.</Info>

Ejemplo 2:
<Info>`${age} > 3 and ${age} < 5` devolverá `true` si la edad está entre 3 y 5.</Info>

Ejemplo 3:
<Info>`not(${age} > 3 and ${age} < 5)` devolverá `true` si la edad no está entre 3 y 5.</Info>
