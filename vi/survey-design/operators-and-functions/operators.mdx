...
Ví dụ 1:
<Info>`${age} = 3 or ${age} = 4` sẽ trả về `true` nếu tuổi là 3 hoặc 4.</Info>

Ví dụ 2:
<Info>`${age} > 3 and ${age} < 5` sẽ trả về `true` nếu tuổi nằm trong khoảng từ 3 đến 5 (không bao gồm 3 và 5).</Info>

Ví dụ 3:
<Info>`not(${age} > 3 and ${age} < 5)` sẽ trả về `true` nếu tuổi không nằm trong khoảng từ 3 đến 5.</Info>
