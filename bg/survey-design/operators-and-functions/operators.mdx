...
Example 1:
<Info>`${age} = 3 or ${age} = 4` will return `true` if the age is either 3 or 4.</Info>

Example 2:
<Info>`${age} > 3 and ${age} < 5` will return `true` if the age is between 3 and 5.</Info>

Example 3:
<Info>`not(${age} > 3 and ${age} < 5)` will return `true` if the age is not between 3 and 5.</Info>
