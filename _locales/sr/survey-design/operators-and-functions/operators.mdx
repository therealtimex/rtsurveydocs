...
Primer 1:
<Callout type="info">`${age} = 3 or ${age} = 4` će vratiti `tačno` ako je godine 3 ili 4.</Callout>

Primer 2:
<Callout type="info">`${age} > 3 and ${age} < 5` će vratiti `tačno` ako su godine između 3 i 5.</Callout>

Primer 3:
<Callout type="info">`not(${age} > 3 and ${age} < 5)` će vratiti `tačno` ako godine nisu između 3 i 5.</Callout>
