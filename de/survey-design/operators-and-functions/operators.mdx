...
Beispiel 1:
<Info>`${age} = 3 or ${age} = 4` gibt `wahr` zurück, wenn das Alter entweder 3 oder 4 ist.</Info>

Beispiel 2:
<Info>`${age} > 3 and ${age} < 5` gibt `wahr` zurück, wenn das Alter zwischen 3 und 5 liegt.</Info>

Beispiel 3:
<Info>`not(${age} > 3 and ${age} < 5)` gibt `wahr` zurück, wenn das Alter nicht zwischen 3 und 5 liegt.</Info>
