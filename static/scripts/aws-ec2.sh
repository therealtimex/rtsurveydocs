...
#   2. In EC2 launch wizard: Advanced Details > User data > paste this entire script.
#   3. Choose Ubuntu Server 22.04 LTS (HVM) as the AMI.
#   4. Recommended: t3.medium (2 vCPU / 4 GB RAM) or larger with Keycloak.
#   5. Security Group: allow SSH (22), HTTP (80), HTTPS (443), Shiny (3838).
#
# Monitor progress:
#   ssh ubuntu@<instance-ip> sudo tail -f /var/log/rtcloud-setup.log
#
...
# Detect public IP (AWS EC2 metadata — IMDSv2)
...
SERVER_IP="${SERVER_IP:-$(curl -s https://api.ipify.org 2>/dev/null || hostname -I | awk '{print $1}')}"
