...
icon: "settings"
toc: true
description: "Volledige referentie voor alle omgevingsvariabelen die worden gebruikt voor het configureren van een zelf-gehoste rtCloud-implementatie."
---

Alle configuratie wordt gedaan via omgevingsvariabelen in het `.env`-bestand in de root van uw implementatiemap. Docker Compose leest dit bestand automatisch — geen `--env-file`-vlag nodig.
