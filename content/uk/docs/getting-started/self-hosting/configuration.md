...
icon: "settings"
toc: true
description: "Повний довідник усіх змінних середовища для налаштування самостійно розміщеного розгортання rtCloud."
---

Вся конфігурація виконується через змінні середовища у файлі `.env` у корені директорії розгортання. Docker Compose читає цей файл автоматично — прапор `--env-file` не потрібен.
