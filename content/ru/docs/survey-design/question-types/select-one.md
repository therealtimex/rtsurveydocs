...
| `tagging` | Отображает варианты как кликабельные чипы-теги вместо переключателей |
| `boxtag` | Отображает варианты как стилизованные прямоугольные блоки, которые пользователь нажимает для выбора |
| `boxtag -search` | Макет boxtag с полем поиска/фильтра над блоками |
| `duolingo-style1` | Карточный макет в стиле Duolingo — большие нажимаемые карточки с иконками |
| `rating_box` | Сетчатые блоки оценки — лучше всего подходят для числовых или шкальных вариантов |
| `star_rating` | Виджет звёздного рейтинга — варианты отображаются как 1–N звёзд |
| `choices-noshow` | Изначально показывает только первые 10 вариантов; остальные показываются по запросу |
| `noshow` | Полностью скрывает список вариантов; значение устанавливается программно |
| `checkall` | Добавляет опцию «Выбрать все» в верхней части списка |
| `max-items(N)` | Ограничивает количество видимых вариантов до N (например, max-items(5)) |
{{< /table >}}
