...
| `required-but-simplify` | любой | Поле обязательно, но макет остаётся упрощённым |
| `embed` | любой | Отображает поле во встроенном/строчном режиме |
| `popup` | select_one, select_multiple | Отображает список вариантов во всплывающем окне/модальном overlay |
| `auto-hide-empty` | boxtag, select | Скрывает виджет, когда список вариантов пуст |
| `text-nolabel` | select_one, select_multiple | Скрывает текстовую метку для каждого варианта |
...
### Визуальные виджеты выбора

| Атрибут внешнего вида | Типы вопросов | Описание |
|----------------------|----------------|-------------|
| `tagging` | select_one, select_multiple | Отображает варианты как кликабельные чипы-теги |
| `boxtag` | select_one, select_multiple | Отображает варианты как стилизованные прямоугольные блоки |
| `boxtag -search` | select_one, select_multiple | Макет boxtag с полем поиска/фильтра над блоками |
| `duolingo-style1` | select_one, select_multiple | Крупный карточный макет — лучше всего для коротких списков вариантов |
| `rating_box` | select_one, select_multiple | Сетчатые блоки оценки |
| `star_rating` | select_one | Виджет звёздного рейтинга — варианты отображаются как 1–N звёзд |
| `choices-noshow` | select_one, select_multiple | Изначально показывает только первые 10 вариантов; остальные по запросу |
| `noshow` | select_one, select_multiple | Полностью скрывает список вариантов; значение устанавливается программно |
| `checkall` | select_multiple | Добавляет опцию «Выбрать все» в верхней части списка |
| `max-items(N)` | select_one, select_multiple | Ограничивает количество видимых вариантов до N |

### Визуальные виджеты для текста

| Атрибут внешнего вида | Типы вопросов | Описание |
|----------------------|----------------|-------------|
| `richtext` | text | Редактор форматированного текста — панель инструментов с жирным, курсивом, списками и ссылками |
| `typingtest` | text | Интерфейс теста на печать — предлагает отрывок и измеряет скорость и точность набора |

### Медиарасширения

| Атрибут внешнего вида | Типы вопросов | Описание |
|----------------------|----------------|-------------|
| `watermark("expression")` | image | Накладывает текстовый водяной знак на захваченное фото; выражение вычисляется в момент захвата |
| `editable` | image | Позволяет респонденту делать аннотации и рисовать на фото после съёмки |

### Конфигурация встроенного отображения

Параметр `display{}` настраивает встроенное отображение:

| Параметр | Описание |
|----------|-------------|
| `display{title="метка"}` | Задаёт пользовательский заголовок для встроенного отображения |
| `display{icon="icon_name"}` | Задаёт иконку для встроенного отображения |
| `display{color="RRGGBB"}` | Задаёт пользовательский цвет для встроенного отображения |

Параметр `results{}` настраивает отображение результатов:

| Параметр | Описание |
|----------|-------------|
| `results{format="pattern"}` | Задаёт шаблон формата для отображения результатов |
| `results{show=true}` | Отображает результаты встроенно |
