...
> **방화벽이 없는 이유는 무엇입니까?** 설정 스크립트에는 아웃바운드 인터넷 액세스(Docker 풀, Let's Encrypt)가 필요합니다. 처음 부팅하는 동안 포트를 차단하면 배포가 실패할 수 있습니다. 설정이 완료된 후 방화벽을 연결할 수 있습니다. 올바른 규칙은 아래의 [방화벽 규칙](#firewall-rules-linode-cloud-firewall)을 참조하세요.
...
3. **Launch LISH Console**(Linode 세부정보 페이지 오른쪽 상단)을 클릭하세요.
...
| 라벨 | 액션 | 메모 |
|-------|---------|-------|
| 기본 아웃바운드 정책 | **수락** | 모든 아웃바운드 허용(Docker 풀, certbot, GoDaddy API 등) |
...
| 8090 | Keycloak 컨테이너 | Nginx는 내부적으로 프록시를 사용합니다 |
| 3306 | MySQL | 내부 Docker 네트워크 전용 |
