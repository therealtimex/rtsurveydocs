...
icon: "lock"
toc: true
description: "为您的 rtSurvey 服务器配置 HTTPS。登录前必须完成。"
---

SSL 必须在登录前配置。首次打开应用程序时，您将自动重定向到 SSL 设置页面。
...
| Option | When to use |
|--------|-------------|
| **免费 rtsurvey.com 子域名 *(推荐)*** | No DNS setup needed. We create the record for you. Ready in 2–5 minutes. |
| **我自己的域名** | You already have a domain and its DNS points to this server. |
| **手动安装证书** | Enterprise or custom CA. Requires SSH access. |

---

## Option 1 — 免费 rtsurvey.com 子域名 *(推荐)*

This is the fastest option. No domain registration or DNS changes required.

1. Click **免费 rtsurvey.com 子域名 *(推荐)*** to expand the section
...
1. Click **我自己的域名** to expand the section
...
1. In the SSL setup screen, click **手动安装证书**
