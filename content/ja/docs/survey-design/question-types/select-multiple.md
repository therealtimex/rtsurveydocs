...
| `tagging` | 選択肢がピル状のタグチップとしてレンダリングされる |
| `boxtag` | 選択肢がスタイル付きの長方形ボックスとしてレンダリングされる |
| `boxtag -search` | ボックスの上に検索/フィルター入力があるBoxtag レイアウト |
| `duolingo-style1` | 大型カードレイアウト — 短い選択肢リストに最適 |
| `rating_box` | グリッドベースの評価ボックス |
| `choices-noshow` | 最初に最初の10個の選択肢を表示；残りはオンデマンドで表示 |
| `checkall` | リストの上部に「すべて選択」オプションを追加する |
| `max-items(N)` | 表示される選択肢の数をNに制限する |
{{< /table >}}
